...
class Story < ApplicationRecord
  ...
  def fetched_attributes
    ...
    unless @fetched_content
      begin
        s = Sponge.new
        s.timeout = 3
        options = { 'User-agent' => "#{Rails.application.domain} for #{fetching_ip}" }
        @fetched_content = s.fetch(url, :get, nil, nil, options, 3)
