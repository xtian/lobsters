...
class CommentsController < ApplicationController
  ...
  def show
    unless (comment = find_comment) && comment.is_editable_by_user?(@user)
  ...
  def reply
    unless (parent_comment = find_comment)
      return render plain: "can't find comment", status: :bad_request
    end
  ...
  def delete
    unless (comment = find_comment) && comment.is_deletable_by_user?(@user)
      return render plain: "can't find comment", status: :bad_request
    end
  ...
  def undelete
    unless (comment = find_comment) && comment.is_undeletable_by_user?(@user)
      return render plain: "can't find comment", status: :bad_request
    end
  ...
  def disown
    unless (comment = find_comment) && comment.is_disownable_by_user?(@user)
      return render plain: "can't find comment", status: :bad_request
    end
  ...
  def downvote
    ...
    unless Vote::COMMENT_REASONS[params[:reason]]
      return render plain: 'invalid reason', status: :bad_request
    end

    unless @user.can_downvote?(comment)
